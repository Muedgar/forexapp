...
import Createwithdrawing from '../forms/transactions_create_withdrawing'
import Deletewithdrawing from '../forms/transactions_delete_withdrawing'
import { getwithdrawings, getwithdrawingsInRange } from '@/app/transactions/withdrawing/actions'


export default function Withdrawing() {
  ...
  const fetchExchanges = async () => {
    const data:any = await getwithdrawings();
    setExchanges(data);
  }
  useEffect(() => {
    fetchExchanges();
  ...
  const handleFilter = async () => {
    if(startDate && endDate) {
      const data:any = await getwithdrawingsInRange(startDate, endDate)
      setExchanges(data)
    }
  }
    ...
    <>
            ...
            <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
              ...
              >
                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                  {id?<Createwithdrawing id={id} getData={fetchExchanges} onClose={setOpen} />:<Createwithdrawing getData={fetchExchanges} />}
                </Dialog.Panel>
              </Transition.Child>
            </div>
          </div>
        ...
        <Dialog as="div" className="relative z-10" initialFocus={cancelButtonRefDelete} onClose={setOpenDelete}>
          ...
          <div className="fixed inset-0 z-10 overflow-y-auto">
            <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
              ...
              >
                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                  <Deletewithdrawing id={id} getData={fetchExchanges} onClose={setOpenDelete}/>
                </Dialog.Panel>
              </Transition.Child>
            </div>
          </div>
      ...
      <>
        ...
        <div className="sm:flex-auto">
          ...
          <h1 className="text-xl font-semibold text-gray-900">Transactions - Withdrawing.</h1>
          <p className="mt-2 text-sm text-gray-700">
            A list of all records of transactions about withdrawing money.
          </p>
        </div>
